...
export async function setarEmbedding(index: number, embedding: number[]) {
  produtos[index].embedding = embedding
}

/**
 * COMO ESTAMOS USANDO ARRAY PARA SALVAR AS INFORMAÇÕES
 * TEMOS QUE CRIAR MANUALMENTE A FUNÇÃO PARA FAZER A 
 * COMPARAÇÃO DE SIMILARIDADE, NO BANCO JÁ TEM FUNÇÕES
 * PRONTAS QUE PODEM SER UTILIZADAS
 */
...
// RETORNANDO OS PRODUTOS QUE TEM SIMILARIDADE
export async function produtosSimilares(embedding: number[]) {
  return produtos.filter(product => product.embedding).map(product => ({
    ...product,
    similaridade: cosineSimilarity(product.embedding!, embedding)
  })).sort((a, b) => b.similaridade - a.similaridade).slice(0, 10)
}
